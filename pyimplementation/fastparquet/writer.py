...
def encode_plain_global(vals,global_dict, sizediff, diffvals, se):
    ...
    for i in range(len(sdictvals)):
        sdictvals[i] = sdictvals.encode('utf-8')
...
def encode_plain(vals, format, global_dict, sizediff, diffvals, distvals, compression, se):
    """PLAIN encoding; returns byte representation"""
    if format not in ['ADAPTIVE','LOCAL']:
...
#        sdictvals[i] = sdictvals[i].encode('utf_8')
    ...
    if format == "LOCAL":
        ...
        diffdict = 0
    elif  lgdict > 0 and ldiff*1.0/ll > 0.99:
        diffdict = 0
        ...
        diffs = 0
        if lgdict+ldiff < 256: diffs = 1
        elif lgdict+ldiff < 65536: diffs= 2
        else: diffs = 4
        
        locpacked = msgpack.dumps(sdictvals)
        sizelocal = len(locpacked) + lenvals3*locs
        sizeofdiff = diffdictdump + lenvals3*diffs
        
        #pblocks (projected blocks until cache is full, TODO calculate this) pblocks = ((          m  - global_dict_memory) / (diffdictdump));
        pblocks = 2000
        if (diffcount <= 1):
            if (sizelocal < sizeofdiff):
                diffdict  = 0;
        		
        elif pblocks * (diffs * len(vals)) -  round(pblocks*1.0/(diffcount)) * sum(sizediff) > 0:
            diffdict = 0
    ...
    #np.save(output, np.array([coldict[y] for y in vals], dtype=type1))
        ...
        output.seek(0)
        output.write(struct.pack(type, *headindex))
        sizediff.append(headindex[0]+headindex[1])
        return output.getvalue()
...
def encode_plain_parquet(data, se):
    """PLAIN encoding; returns byte representation"""
    output = StringIO()
    headindex = [0]*7
    type = 'i'*len(headindex)
...
def make_definitions(data,format, global_dictionary, sizediff, diffvals, distvals, compression, no_nulls):
    ...
    if no_nulls:
        ...
        out = encode_plain(data.notnull(),format, global_dictionary, sizediff, diffvals,distvals, compression, se)
...
def write_column(f,format, global_dictionary, sizediff, diffvals,distvals, data, selement, compression=None):
    """
    ...
    if has_nulls:
        ...
        definition_data, data = make_definitions(data,format, global_dictionary, sizediff, diffvals,distvals,compression, num_nulls == 0)
    ...
    if is_categorical_dtype(data.dtype):
        ...
        bdata = encode['PLAIN'](pd.Series(data.cat.categories),format, global_dictionary, sizediff, diffvals,distvals, selement)
        bdata += 8 * b'\x00'
        l0 = len(bdata)
        if compression:
            ...
            if num_nulls == 0:
                ...
                if selement.type == parquet_thrift.Type.BYTE_ARRAY:
                    if selement.converted_type is not None:
                        max = encode['PLAIN'](pd.Series([max]), format, global_dictionary, sizediff, diffvals,distvals, selement)[4:]
                        min = encode['PLAIN'](pd.Series([min]),format, global_dictionary, sizediff, diffvals,distvals, selement)[4:]
                else:
                    max = encode['PLAIN'](pd.Series([max]),format, global_dictionary,sizediff, diffvals, distvals,selement)
                    min = encode['PLAIN'](pd.Series([min]),format, global_dictionary, sizediff, diffvals, distvals,selement)
        except TypeError:
            pass
    ...
    bdata = encode[encoding](
            data,format, global_dictionary, sizediff, diffvals,distvals, compression, selement)
...
def make_row_group( f, format, global_dictionary, sizediff, diffvals, distvals, data, schema, compression=None):
    ...
    for i,column in enumerate(schema):
        if column.type is not None:
            ...
            t1 = f.tell()
            chunk = write_column(f,format, global_dictionary[i-1], sizediff[i-1], diffvals[i-1],distvals[i-1], data[column.name], column,
                                 compression=comp)
...
def write_simple(fn,format, global_dictionary, sizediff, diffvals, distvals, data, fmd, row_group_offsets, chunksize, compression,
    ...
    with open_with(fn, mode) as f:
        ...
        for i, start in enumerate(row_group_offsets):
            ...
            rg = make_row_group(f, format, global_dictionary, sizediff, diffvals, distvals, data[start:end], fmd.schema,
...
def write(filename, data, row_group_offsets=50000000,
          compression=None, format=None, file_scheme='simple', open_with=default_open,
    ...
    if file_scheme == 'simple':
        write_simple(filename,format, global_dictionary, sizediff, diffvals,distvals, data, fmd, row_group_offsets, chunksize,
