...
def encode_plain(blocknum, vals, format, global_dict, sizediff, blocksizes, diffvals, distvals, compression, se):
    ...
    if format == "LOCAL":
        ...
        pblocks =  ((8192000*2 - (sum(sizediff)+len(locpacked))) / (diffdictdump));
        if (diffcount <= 1):
    ...
    #np.save(output, np.array([coldict[y] for y in vals], dtype=type1))
        ...
        l3 = output.tell()
        headindex[1] = l3-l2
        blocksizes.append(headindex[1]+headindex[0])
        output.seek(0)
        output.write(struct.pack(type, *headindex))
        return output.getvalue()
        ...
        l10 = output.tell()
        output.write(msgpack.dumps(minmaxdiff))
        l11 = output.tell()
        headindex[8]=l11-l10
        headindex[5]=0
        point = lgdict
...
def encode_plain1(vals,global_dict, sizediff, diffvals, distvals,se):
...
def write_column(blocknum, f,format, global_dictionary, sizediff, blocksizes, diffvals,distvals, data, selement, compression=None):
    ...
    if is_categorical_dtype(data.dtype):
        ...
        bdata = encode['PLAIN'](blocknum,pd.Series(data.cat.categories),format, global_dictionary, sizediff,blocksizes, diffvals,distvals, selement)
        #bdata += 8 * b'\x00'
        l0 = len(bdata)
        if compression:
            ...
            if num_nulls == 0:
                ...
                if selement.type == parquet_thrift.Type.BYTE_ARRAY:
                    if selement.converted_type is not None:
                        max = encode['PLAIN'](blocknum,pd.Series([max]), format, global_dictionary, sizediff,blocksizes, diffvals,distvals, selement)[4:]
                        min = encode['PLAIN'](pd.Series([min]),format, global_dictionary, sizediff,blocksizes, diffvals,distvals, selement)[4:]
                else:
                    max = encode['PLAIN'](blocknum,pd.Series([max]),format, global_dictionary,sizediff,blocksizes, diffvals, distvals,selement)
                    min = encode['PLAIN'](blocknum,pd.Series([min]),format, global_dictionary, sizediff,blocksizes, diffvals, distvals,selement)
        except TypeError:
            pass
        ...
        for i, start in enumerate(row_group_offsets):
            ...
            if partition_on:
                rgs = partition_on_columns(
                    data[start:end],global_dictionary, sizediff,blocksizes, diffvals, partition_on, filename, part, fmd,
...
def partition_on_columns(data, global_dictionary, sizediff,blocksizes, diffvals, columns, root_path, partname, fmd,
    ...
    for key, group in zip(sorted(gb.indices), sorted(gb)):
        ...
        with open_with(fullname, 'wb') as f2:
            rg = make_part_file(f2,global_dictionary, sizediff,blocksizes, diffvals, df, fmd.schema,
                                compression=compression, fmd=fmd)
        if rg is not None:
            for chunk in rg.columns:
