...
int compress_batch2(vector<string> &vals,vector<string> &globaldict,vector<string> &glob, FILE *f1, bloom_filter *filter, bool &isdictionary,vector<int> &blocksize, vector<int> &sizediff,
...
if (diffdict == 1){
    int kkk = glob.size();
    glob.insert(glob.end(), diff.begin(), diff.end());
    inplace_merge (glob.begin(),glob.begin()+kkk,glob.end());
